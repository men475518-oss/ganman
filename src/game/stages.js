...
G.stages = (function () {
  ...
  /* ------------------------------------------------------------------
     敵の解禁段階（ステージを進むほど強い敵が出てくる）

       段階0 … 基本の敵。最初から出る
       段階1 … 少し厄介な敵。3割ほど進んでから
       段階2 … 硬い・堅実に対処が要る敵。6割ほど進んでから

     序盤は「一個前の顔ぶれ」だけで肩慣らしをして、
     進むにつれて新顔が混ざり、数も増えていくようにしている。
     ------------------------------------------------------------------ */
  var TIER = {
    met: 0, fly: 0, turret: 0, hop: 0, spike: 0,
    crawl: 1, bat: 1, split: 1, riser: 1,
    joe: 2, tank: 2
  };
  var TIER_AT = [0, 0.30, 0.58];    // 各段階が解禁される進行度

  /* ステージのどのあたりまで来たか（0=開始, 1=ボス手前） */
  function progress(S) {
    return U.clamp((S.x - 12) / Math.max(1, S.endX - 12), 0, 1);
  }

  /* 進行度に応じた敵の抽選。
     解禁済みの段階だけを候補にし、新しい段階ほど後半で出やすくする。 */
  ...
  /* 区間内に敵をばらまく。数は進行度に比例して増える
     （序盤は指定の約0.5倍、終盤は約1.6倍）           */
  function populate(S, x0, x1, groundRow, n) {
    ...
    for (var i = 0; i < count; i++) {
      var tx = x0 + Math.floor((i + 0.5) / count * span) + U.rndInt(-1, 1);
  ...
  /* このパターンは「進行度がこれ以上」でないと出さない。
     強い敵を連れてくるものや、難しいものを序盤に置かないための制限。 */
  var PATTERN_MIN_P = {
    guardPost: 0.42,     // 盾持ち兵が出る
    crawlBlocks: 0.22,   // 壁這いが出る
    crusherHall: 0.28,
    ventRun: 0.22,
    bigPit: 0.16,
    movingGap: 0.20,
